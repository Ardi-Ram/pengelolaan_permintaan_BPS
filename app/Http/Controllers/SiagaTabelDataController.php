...
class SiagaTabelDataController extends Controller
{
    ...
    public function simpanFinal(Request $request)
    {
        ...
        $request->validate([
            ...
            'data.required' => 'Tidak ada data untuk disimpan.',
            'data.array' => 'Format data tidak valid.',
            'data.*.judul_publikasi.required' => 'Judul publikasi tidak boleh kosong pada baris :attribute.',
            'data.*.judul_tabel.required' => 'Judul tabel tidak boleh kosong pada baris :attribute.',
        ...
        // Hitung jumlah baris yang punya judul_publikasi dan judul_tabel yang tidak kosong
        $validRows = collect($inputData)->filter(function ($row) {
            return !empty(trim($row['judul_publikasi'] ?? '')) && !empty(trim($row['judul_tabel'] ?? ''));
        })->count();
        ...
        foreach ($inputData as $index => $row) {
            ...
            Log::debug("Row ke-$index", [
                'judul_publikasi' => $judul_publikasi,
                'judul_tabel' => $judul_tabel
            ]);

            if (empty($judul_publikasi) || empty($judul_tabel)) {
                Log::warning("simpanFinal: Melewatkan baris ke-$index karena 'judul_publikasi' atau 'judul_tabel' kosong.", ['row_data' => $row]);
            ...
            try {
                ...
                Log::error('simpanFinal: Gagal menyimpan baris data ke-' . $index . ': ' . json_encode($row) . ' - Error: ' . $e->getMessage());
        ...
        if ($savedCount > 0) {
            ...
            if (!empty($failedRows)) {
                ...
                // Anda bisa menambahkan detail failedRows ke session untuk ditampilkan di view jika diperlukan
            ...
            if (empty($inputData)) {
                return redirect()->route('siaga.import.form')->with('error', 'Tidak ada data yang dikirim dari form preview.');
            } else {
                return redirect()->route('siaga.import.form')->with('error', 'Tidak ada data valid yang dapat disimpan dari ' . count($inputData) . ' baris yang dikirim. Periksa log untuk detailnya.');
            }
        }
    }
    ...
    public function halamanPenugasan(Request $request)
    {
        if ($request->ajax()) {
            $data = SiagaTabelData::select('judul_publikasi', 'nomor_publikasi', DB::raw('MAX(created_at) as created_at'))
                ...
                ->where('status', '!=', 'rilis') // ⬅️ tambahkan ini
            ...
            return DataTables::of($data)
                ->addColumn('status', function ($row) {
                    ...
                    return ($ditugaskan === $total)
                        ? '<span class="px-2 py-1 text-xs rounded bg-green-500 text-white">Sudah Ditugaskan</span>'
                        : '<span class="px-2 py-1 text-xs rounded bg-yellow-400 text-white">Belum Ditugaskan</span>';
                })

                // ✅ Tambahkan kolom Petugas PST
                ->addColumn('petugas_pst', function ($row) {
                    ...
                    if ($first && $first->petugasPst) {
                        return '<div class="text-sm font-semibold text-gray-800">' . e($first->petugasPst->name) . '</div>' .
                            '<div class="text-xs text-gray-500">' . e($first->petugasPst->email) . '</div>';
                    }

                    return '<span class="text-xs text-gray-400 italic">Belum Ditugaskan</span>';
                })

                // ✅ Tambahkan ikon pada tombol aksi
                ->addColumn('aksi', function ($row) {
                    ...
                    $btnDetail = '<button class="btn-detail flex items-center gap-1 text-blue-600 underline text-sm" data-judul="' . $judul . '">
                    <span class="material-symbols-outlined text-base">info</span> Detail
                </button>';

                    if ($ditugaskan === $total) {
                        $btnBatal = '<button class="btn-batal flex items-center gap-1 bg-red-600 hover:bg-red-700 text-white px-3 py-1 rounded text-sm"
                        data-judul="' . $judul . '">
                        <span class="material-symbols-outlined text-base">cancel</span> Batalkan
                    </button>';
                        return '<div class="flex flex-wrap gap-2">' . $btnDetail . $btnBatal . '</div>';
                    } else {
                        $btnTugaskan = '<button class="btn-modal flex items-center gap-1 bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded text-sm"
                        data-judul="' . $judul . '">
                        <span class="material-symbols-outlined text-base">assignment_ind</span> Tugaskan
                    </button>';
                        return '<div class="flex flex-wrap gap-2">' . $btnDetail . $btnTugaskan . '</div>';
                    }
                })

                ->rawColumns(['status', 'petugas_pst', 'aksi'])
                ->make(true);
        }
    ...
    public function dataUntukPst(Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                // ✅ Kolom nama pengolah
                ->addColumn('pengolah', function ($row) {
                    $pengolah = SiagaTabelData::where('judul_publikasi', $row->judul_publikasi)
                        ->with('pengolah')
                        ->first()?->pengolah?->name;

                    return $pengolah ?? '<span class="italic text-gray-400">Belum ditugaskan</span>';
                })

                // ✅ Kolom Aksi dengan button style & icon Google
                ->addColumn('aksi', function ($row) {
                    $judul = e($row->judul_publikasi);
    ...
    public function uploadLink(Request $request)
    {
        $request->validate([
            'judul_publikasi' => 'required|string',
            'link' => 'required|url' // Validasi URL
        ]);
    ...
    public function portalData(Request $request)
    {
        ...
        return DataTables::of($query)
            ->addIndexColumn()
            ->addColumn('link_output', function ($row) { // Menggunakan function($row) untuk kejelasan
                // Periksa apakah link_output ada
                if ($row->link_output) {
                    // Gunakan kelas Tailwind yang benar untuk button (bukan gradient jika hanya solid color)
                    return '<a href="' . $row->link_output . '" target="_blank" class="inline-flex items-center px-3 py-1.5 bg-blue-700 text-white text-sm font-semibold rounded-lg hover:bg-blue-800 transition-all duration-200 shadow-sm hover:shadow-md">
                                Unduh
                            </a>';
                }
                // Jika tidak ada link_output, kembalikan teks "Tidak tersedia"
                return '<span class="text-gray-400 italic">Tidak tersedia</span>';
            })
            ->rawColumns(['link_output']) // Ini penting untuk merender HTML
            ->make(true);
    }
}
