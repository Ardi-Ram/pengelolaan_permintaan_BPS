...
@section('content')
    <div class="max-w-xl mx-auto px-4 py-6">
        ...
        <form action="{{ route('faq.update', $faq) }}" method="POST" class="space-y-4">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="answer" rows="4" class="w-full border rounded px-3 py-2" required>{{ old('answer', $faq->answer) }}</textarea>
                @error('answer')
                    <p class="text-red-600 mt-1">{{ $message }}</p>
                @enderror
            </div>
